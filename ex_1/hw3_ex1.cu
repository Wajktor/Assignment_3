...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    int index_out = index_x + width * index_y;
    int index_in  = 3*index_out;

    float *pixel = &image[index_in]; 
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
 ...
 /*   if(threadIdx.x == 0 && threadIdx.y == 0){
        sh_block[0] = image[offset];
    }

    if(threadIdx.x == 15 && threadIdx.y == 0){
        sh_block[17] = image[offset + 2];
    }

    if(threadIdx.x == 15 && threadIdx.y == 15){
        sh_block[18*18-1] = image[offset + 2 + width];
    }
...
/*
        if(blockIdx.y == 0 && threadIdx.x == 10 && threadIdx.y == 0){
            //printf("sh_block[%d] : %f \n", (threadIdx.x + threadIdx.y * blockDim.x), sh_block[threadIdx.x + threadIdx.y * blockDim.x]);
            //printf("image[[%d]] : %d \n", offset_t, *(image+offset_t));
            //printf("blockX: %d, blockY: %d, threadX: %d, threadY: %d \n", blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y);
            //printf("image[[%d]] : %f \n", offset_t, image[offset_t]);
        }
